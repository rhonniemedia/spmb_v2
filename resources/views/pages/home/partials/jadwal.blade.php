<section id="jadwal" class="py-24">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Desktop: tabel biasa --}}
        <div class="glass rounded-3xl overflow-hidden fade-up hidden md:block">
            <table class="w-full">
                <thead>
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="jadwal-row transition-colors cursor-default">
                        ...
                        <td class="px-6 py-5 text-slate-300 text-sm">{{ $step->period_text }}</td>
                        <td class="px-6 py-5">
                            @if($isOpen)
        ...
        {{-- Mobile: card list --}}
        <div class="flex flex-col gap-3 md:hidden fade-up">
            @foreach($spmbSteps as $step)
            @php
            $c = $step->color ?? 'cyan';
            $currentDate = now();
            ...
            <div class="glass rounded-2xl px-4 py-4">
                <div class="flex items-start justify-between gap-3">
                    ...
                    <div class="flex items-start gap-3 flex-1 min-w-0">
                        <div class="w-9 h-9 rounded-xl bg-{{ $c }}-400/10 flex items-center justify-center shrink-0 mt-0.5">
                            <i class="fa-solid {{ $step->icon ?? 'fa-circle-dot' }} text-{{ $c }}-400 text-sm"></i>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="font-semibold text-white text-sm leading-snug">{{ $step->title }}</p>
                            ...
                            <div class="flex items-center gap-1.5 mt-2">
                                ...
                                <span class="text-slate-400 text-xs font-medium">{{ $step->period_text }}</span>
                            </div>
                        </div>
                    </div>
                    {{-- Status badge --}}
                    <div class="shrink-0 mt-0.5">
                        @if($isOpen)
                        <span class="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full bg-green-400/10 text-green-400 border border-green-400/20 font-semibold whitespace-nowrap">
                            <span class="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></span>Berlangsung
                        </span>
                        @elseif($isPast)
                        <span class="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full bg-red-400/10 text-red-400 border border-red-400/20 font-semibold">
                            Selesai
                        </span>
                        @else
                        <span class="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full bg-slate-100/50 text-slate-400 border border-slate-600/30 font-semibold whitespace-nowrap">
                            Menunggu
                        </span>
                        @endif
                    </div>
