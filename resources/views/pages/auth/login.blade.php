@extends('layouts.auth')

@section('title', 'Login')

@section('content')

<div class="flex-1 flex items-center justify-center p-6 py-12">
  <div class="w-full max-w-md">

    <!-- Mobile logo -->
    <div class="flex justify-center mb-8 lg:hidden fade-up">
      <a href="index.html" class="flex items-center gap-3">
        <div class="logo-badge"><i class="fa-solid fa-graduation-cap text-white text-xl"></i></div>
        <div>
          <p style="font-weight:800;font-size:0.95rem;color:var(--foreground);">SMK Negeri 1</p>
          <p style="font-size:0.72rem;color:var(--secondary);">Rejang Lebong</p>
        </div>
      </a>
    </div>

    <!-- Card -->
    <div class="auth-card p-8 fade-up delay-1">

      <!-- Header -->
      <div class="mb-8">
        <h2 style="font-size:1.6rem;font-weight:800;color:var(--foreground);margin-bottom:6px;">Masuk ke Akun</h2>
        <!-- Register link bawah -->
        <p style="font-size:0.88rem;color:var(--secondary);">Belum punya akun? <a href="{{ route('register') }}" style="color:var(--primary);font-weight:600;text-decoration:none;">Daftar Sekarang →</a></p>
      </div>

      <!-- Alert Error -->
      <div class="alert-error mb-5" id="alertError">
        <i class="fa-solid fa-circle-exclamation"></i>
        <span id="alertMsg">Email atau password salah.</span>
      </div>

      <!-- Form -->
      <form onsubmit="handleLogin(event)" class="space-y-5">

        <!-- Email -->
        <div class="fade-up delay-3">
          <label style="display:block;font-size:0.83rem;font-weight:600;color:var(--foreground);margin-bottom:8px;">Email</label>
          <div class="input-wrapper">
            <i class="fa-solid fa-envelope input-icon"></i>
            <input type="email" id="loginEmail" class="input-field" placeholder="contoh@email.com" required />
          </div>
        </div>

        <!-- Password -->
        <div class="fade-up delay-3">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <label style="font-size:0.83rem;font-weight:600;color:var(--foreground);">Password</label>
            <a href="#" style="font-size:0.8rem;color:var(--primary);font-weight:600;text-decoration:none;">Lupa password?</a>
          </div>
          <div class="input-wrapper">
            <i class="fa-solid fa-lock input-icon"></i>
            <input type="password" id="loginPassword" class="input-field" placeholder="Masukkan password" required style="padding-right:48px;" />
            <button type="button" class="toggle-password" onclick="togglePwd('loginPassword', this)">
              <i class="fa-solid fa-eye"></i>
            </button>
          </div>
        </div>

        <!-- Remember me -->
        <div class="flex items-center gap-3 fade-up delay-4">
          <input type="checkbox" id="rememberMe" class="custom-checkbox" />
          <label for="rememberMe" style="font-size:0.85rem;color:var(--secondary);cursor:pointer;">Ingat saya di perangkat ini</label>
        </div>

        <!-- Submit -->
        <div class="fade-up delay-4 pt-1">
          <button type="submit" class="btn-primary" id="btnLogin">
            <span id="btnLoginText">Masuk Sekarang</span>
            <span id="btnLoginSpinner" style="display:none;"><i class="fa-solid fa-circle-notch fa-spin mr-2"></i>Memproses...</span>
          </button>
        </div>
      </form>

      <!-- Divider -->
      <div class="divider my-6 fade-up delay-2">atau masuk dengan</div>

      <!-- Google Button -->
      <button class="btn-google fade-up delay-2" onclick="loginGoogle()">
        <svg width="20" height="20" viewBox="0 0 24 24">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" />
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
        </svg>
        Masuk dengan Google
      </button>

    </div>

    <!-- Back to landing -->
    <p class="text-center mt-3 fade-up delay-5">
      <a href="{{ route('home') }}" style="font-size:0.82rem;color:var(--secondary);text-decoration:none;display:inline-flex;align-items:center;gap:6px;">
        <i class="fa-solid fa-arrow-left" style="font-size:0.75rem;"></i> Kembali ke Beranda
      </a>
    </p>

  </div>
</div>

@endsection