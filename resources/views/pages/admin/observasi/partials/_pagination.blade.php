...
<div class="flex flex-col md:flex-row items-center justify-between gap-4 pt-4">
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
        ...
        @else
        <button type="button" hx-get="{{ $peserta->previousPageUrl() }}" hx-target="#peserta-container"
            hx-select="#peserta-container" hx-swap="outerHTML" hx-push-url="true"
        ...
        ->filter(fn($p) => $p >= 1 && $p <= $last)
            ...
            @else
            <button type="button"
                hx-get="{{ $peserta->url($page) }}"
                hx-target="#peserta-container" hx-select="#peserta-container"
            ...
            <button type="button" hx-get="{{ $peserta->nextPageUrl() }}" hx-target="#peserta-container"
                hx-select="#peserta-container" hx-swap="outerHTML" hx-push-url="true"
                class="p-2 rounded-lg border border-border bg-white hover:bg-muted cursor-pointer transition-colors">
